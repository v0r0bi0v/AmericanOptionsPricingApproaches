...
class LSPIPricer(PricerAbstract):
    ...
    def _basis_functions(self, S: np.ndarray, K: float, t: np.ndarray, T: float) -> np.ndarray:
        ...
        phi = np.zeros((S.shape[0], S.shape[1], 7), dtype=float)
        phi[:, :, 0] = 1.0
        phi[:, :, 1] = exp_term
    ...
    def price(self, test=False) -> np.ndarray:
        ...
        gamma = np.exp(-r * dt)
        S_paths = self.sampler.markov_state
        payoff_matrix = self.sampler.payoff
        disc_matrix = self.sampler.discount_factor
        num_paths, num_times = S_paths.shape[0], S_paths.shape[1]
        d = 7
